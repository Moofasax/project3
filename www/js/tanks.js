...
//sound played to client when they join game (does not play to all users)
var testYourMightAudio = new Audio('js/testYourMight.mp3');
...
Game.prototype = {
	...
	addTank: function(id, name, type, isLocal, x, y, hp){
		...
		if(isLocal){
			this.localTank = t;
			// testYourMightAudio.play();
		}else{
			this.tanks.push(t);
		}
		//DISPLAY USER JOINED GAME HERE
	...
	killTank: function(tank){
		...
		setTimeout(function(){
			$('#expl' + tank.id).remove();
			// ADD THIS TO A DEATH DISPLAY DIV
			//to display the stats of the most recent death players name, uncomment this line
			// $('#statsContainer').html('<p>your total shots: ' + tank.numberShot + '</p>');
		}, 1000);
	...
	receiveData: function(serverData){
		...
		serverData.tanks.forEach( function(serverTank){
			...
			if(game.localTank !== undefined && serverTank.id == game.localTank.id){
				...
				if(game.localTank.hp <= 0){
					// add game stats here to only be displayed to the local user
					$('#statsContainer').html('<p>your total shots: ' + game.localTank.numberShot + '</p>');
					console.log('the ball that killed you');
					console.log(game);
		...
		serverData.balls.forEach( function(serverBall){
			...
			//this signifies a hit on a tank
			if(b.exploding){
				...
				//the user just hit someone
				if(game.localTank.id == b.ownerId){
					console.log("we have just tracked a hit from a local tank.");
				}

				for(var i = 0; i < serverData.tanks; i++){
					if(b.ownerId == serverData.tanks[i]){
						console.log("poop");
					}
				}
				console.log(serverData.tanks);

				//once the ball hits a tank, check if it is a kill shot.
